...
## Phase 28 — إصلاح QR وربط Android من Windows

تم تحسين صفحة `الإعدادات / الشبكة` بحيث يستخدم QR عنوان LAN المختار من الواجهة، وليس عنوانًا عشوائيًا من VPN أو كرت شبكة افتراضي. يظهر الآن اختيار واضح لعنوان الربط، QR أكبر، نص الربط مخفي افتراضيًا، وعدّاد انتهاء بصيغة مفهومة.

اختبار سريع:

1. شغّل الخادم المحلي.
2. افتح `الإعدادات / الشبكة`.
3. اختر عنوان LAN مثل `192.168.x.x`.
4. اضغط `إنشاء رمز ربط للهاتف`.
5. تأكد أن عنوان QR مطابق للعنوان المختار.
