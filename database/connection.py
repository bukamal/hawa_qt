...
class DatabaseConnection:
    ...
    def _use_http(self):
        # استخدام الوضع HTTP إذا تم تعيين SERVER_URL وليس localhost فقط (أو إذا كان الخادم مفعلاً)
    ...
    def get_connection(self):
        if not self._use_http():
            ...
            # وضع HTTP: نستخدم connection_id مخزناً محلياً
            ...
            # نعيد كائن وهمي يتوافق مع واجهة sqlite3 (لكننا سنستخدم execute مباشرة)
            return self._local
    ...
    def execute(self, sql, params=()):
        if not self._use_http():
            ...
            conn_id = self.get_connection().conn_id  # conn هو الكائن المحلي الذي يحمل conn_id
